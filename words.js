...
/* Настоящие фонемы букв — играют при установке буквы в слот/ячейку. Файлы с суффиксом (1). */
const LETTER_PHONEMES={};
'АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЫЭЮЯ'.split('').forEach(ch=>{ LETTER_PHONEMES[ch]='letter_phoneme/'+ch+'(1).mp3'; });
...
/* Разбивка слов на слоги. Есть слово здесь → mode:'syllables'. Нет → mode:'letters' (односложные). */
const WORD_SYLLABLES={
  'аист':['а','ист'], 'еда':['е','да'], 'жарко':['жар','ко'], 'змея':['зме','я'],
  'идея':['и','де','я'], 'йогурт':['йо','гурт'], 'коза':['ко','за'], 'луна':['лу','на'],
  'лето':['ле','то'], 'мама':['ма','ма'], 'море':['мо','ре'], 'молоко':['мо','ло','ко'],
  'нюхать':['ню','хать'], 'обед':['о','бед'], 'остров':['ос','тров'], 'папа':['па','па'],
  'палец':['па','лец'], 'рыба':['ры','ба'], 'танец':['та','нец'], 'топать':['то','пать'],
  'ухо':['у','хо'], 'утюг':['у','тюг'], 'филин':['фи','лин'], 'холод':['хо','лод'],
  'цапля':['ца','пля'], 'цветок':['цве','ток'], 'чашка':['чаш','ка'], 'шагать':['ша','гать'],
  'щетка':['щёт','ка'], 'щипать':['щи','пать'], 'эму':['э','му'], 'этаж':['э','таж'],
  'юла':['ю','ла'], 'юмор':['ю','мор'], 'язык':['я','зык'], 'яблоко':['я','бло','ко']
};
...
const PATH_NODES='АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ'.split('').map(function(ch){
  ...
  return {
    ...
    words:ws.map(function(w){
      var syl=WORD_SYLLABLES[w];
      return syl ? makeSyllWord({w:w, emoji:WORD_EMOJI[w]||'⭐', syl:syl})
                 : makeLetterWord({w:w, emoji:WORD_EMOJI[w]||'⭐'});
    })
  };
});
